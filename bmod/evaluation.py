...
def evaluate(path):
    return test_transcriptions.test_files(
        path,
        config["evaluation"]["ground_truth_easy"],
        config["evaluation"]["ground_truth_medium"],
        config["evaluation"]["ground_truth_hard"])


def write_results(file, results):
    file_id = file.split("/")[-1]
    translation = load_translations(config["evaluation"]["translation_file"])
    ...
    data = load_result_data(config["evaluation"]["result_data"])
    data = update(data, name, description, results)
    write_data(data, config["evaluation"]["result_data"])
